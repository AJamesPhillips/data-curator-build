import {get_attribute_by_index_lookup, get_attributes_by_index_lookup} from "../../objects/object_content.js";
import {date2str} from "../../shared/utils/date_helpers.js";
import {MSECONDS_PER_DAY} from "../../canvas/display.js";
import {memoize} from "../../utils/memoize.js";
import {factory_filter_objects_by_pattern_id_c} from "../../state/objects/filter_objects.js";
const PATTERN_ACTION_V2 = "p9";
const filter_objects_c = factory_filter_objects_by_pattern_id_c(PATTERN_ACTION_V2);
export function get_daily_actions_meta_c(state) {
...
function _get_daily_actions_meta(raw_actions) {
  const actions_by_project_id = {};
  raw_actions.forEach((action) => {
    const {attributes, id: action_id} = action;
    let project_id_attrs = get_attributes_by_index_lookup(1, attributes);
    const start_datetime_attr = get_attribute_by_index_lookup(8, attributes);
    const stop_datetime_attr = get_attribute_by_index_lookup(9, attributes);
    const start_datetime = new Date(start_datetime_attr.value);
    const stop_datetime = new Date(stop_datetime_attr.value);
    if (Number.isNaN(start_datetime.getTime()))
      return;
