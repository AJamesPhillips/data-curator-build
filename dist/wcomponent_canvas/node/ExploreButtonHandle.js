...
function _ExploreButtonHandle(props) {
  ...
  const class_name = `node_handle explore ` + (hidden ? " hidden " : "") + (kvwc ? " has_knowledge_view " : "") + (current_but_no_parent ? " current_but_no_parent " : "") + (click_outcome < 0 ? " disabled " : "") + (click_outcome === ClickOutComes.create_then_navigate ? " will_create_on_click " : "");
