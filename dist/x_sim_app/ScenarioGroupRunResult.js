...
import {round_number} from "../canvas/position_utils.js";
export function ScenarioGroupRunResultComponent(props) {
  ...
  const class_name = "scenario_group_run_result " + (complete ? " complete " : " incomplete ");
  const result_string = useMemo(() => {
    ...
    const result_string2 = `Mean --- Run time (days) ${total_sim_time_mean_days} --- Solvent: ${percent_solvent}% --- Demand met: ${customer_demand_fulfilled_percentage_mean}% --- Company profit ${total_company_profit_mean}$`;
    return result_string2;
  }, [complete]);
  return /* @__PURE__ */ h("div", {
    className: class_name
  }, date2str_auto({date: group_run.started, time_resolution: "second"}), "  ", percent_complete, "%    ", result_string);
}
function as_percent(ratio) {
