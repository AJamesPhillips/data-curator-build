...
import {VAP_visual_false_id} from "../value_and_prediction/utils_to_convert_VAP_set_to_visuals.js";
import {get_default_wcomponent_title} from "./get_default_wcomponent_title.js";
...
function test_rendering_title() {
  ...
  function get_title_for_id(args) {
    const {id, rich_text, render_links, wc_id_to_counterfactuals_map: wc_id_to_counterfactuals_map2} = args;
    return get_title({
      rich_text,
      render_links,
      wcomponents_by_id,
  ...
  Object.entries(expected_rich_text_counterfactual).forEach(([id, expected_title]) => {
    const result2 = get_title_for_id({id, rich_text: true, wc_id_to_counterfactuals_map});
    test(result2, expected_title);
  });
}
function run_tests() {
