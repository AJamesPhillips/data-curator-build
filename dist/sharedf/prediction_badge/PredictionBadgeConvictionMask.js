...
export function PredictionsBadgeConvictionMask(props) {
  ...
  return /* @__PURE__ */ h("g", {
    ...
    className: "parts"
  }, mask_data.map((e) => /* @__PURE__ */ h("rect", {
    "data-group_id": e[2],
...
function generate_predictions_badge_conviction_mask(props) {
  ...
  function get_group() {
    const choices = Object.keys(groups).filter((k) => groups[k] > 0);
  ...
  function get_groupv2() {
    const choices = [];
    Object.keys(groups).forEach((k) => Array.from(Array(groups[k])).forEach((_) => choices.push(k)));
    const index = Math.floor(Math.random() * choices.length);
    const group_id = choices[index];
    groups[group_id] -= 1;
    return group_id;
  }
  return Array.from(Array(total_elements)).map((_, i) => {
