...
import {group_VAP_sets_by_version, sort_grouped_VAP_sets, ungroup_VAP_sets_by_version} from "../../shared/wcomponent/value_and_prediction/utils.js";
import {test} from "../../shared/utils/test.js";
import {upsert_entry, remove_from_list_by_predicate} from "../../utils/list.js";
import {get_summary_for_single_VAP_set, get_details_for_single_VAP_set, get_details2_for_single_VAP_set} from "./common.js";
...
export function ValueAndPredictionSetsComponent(props) {
  ...
  const new_VAP_set_form_top_props = {
  ...
  const title = editing ? get_items_descriptor(item_descriptor, values_and_prediction_sets.length, editing) : item_descriptor;
  ...
  return /* @__PURE__ */ h("div", {
    ...
    item_descriptor,
    add_item: (new_item2) => {
      props.update_items([...values_and_prediction_sets, new_item2]);
      set_new_item(void 0);
    }
  }), invalid_future_items.length || invalid_past_items.length ? /* @__PURE__ */ h("div", null, "Hidden (", invalid_future_items.length, ", ", invalid_past_items.length, ")") : null, show_futures && /* @__PURE__ */ h(ExpandableList, {
...
function validate_sort_and_group_VAP_sets_by_version(values_and_prediction_sets, VAPs_represent) {
...
function validate_VAP_sets_for_VAPs_represent(VAP_sets, VAPs_represent) {
...
function count_and_versions(title, grouped_items, all_versions, editing) {
  if (!editing)
    return title;
...
function factory_render_list_content2(args) {
  ...
  const render_list_content = (list_content_props) => {
    ...
    }, grouped_versioned_VAP_sets.map((item) => /* @__PURE__ */ h("div", {
      key: get_latest_id(item)
    }, /* @__PURE__ */ h("hr", {
...
const predicate_by_id = (i1) => (i2) => {
  return i1.id === i2.id && i1.version === i2.version;
};
const get_latest_id = (item) => item.latest.id;
const get_latest_created_at = (item) => item.latest.created_at;
const get_latest_custom_created_at = (item) => item.latest.custom_created_at;
const set_latest_custom_created_at = (item, custom_created_at) => {
  return {...item, latest: {...item.latest, custom_created_at}};
};
const get_summary = (VAPs_represent, VAP_set_counterfactuals_map) => (versioned_VAP_set, on_change) => {
...
const get_details = (VAPs_represent, wcomponent_id, VAP_set_counterfactuals_map) => (versioned_VAP_set, on_change) => {
  const {latest: latest_VAP_set, older} = versioned_VAP_set;
...
const get_details2 = (VAPs_represent, editing) => (versioned_VAP_set, on_change) => {
  const {latest: latest_VAP_set, older} = versioned_VAP_set;
...
function factory_handle_change(args) {
  const {
    all_VAP_sets,
    item,
    update_items
