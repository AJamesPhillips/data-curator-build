...
export async function save_solid_data(user_info, data) {
  ...
  return save_knowledge_views(solid_pod_URL, data.knowledge_views).then(() => save_wcomponents(solid_pod_URL, data.wcomponents));
}
...
async function save_wcomponents(solid_pod_URL, wcomponents) {
  const wcomponents_url = get_wcomponents_url(solid_pod_URL);
  const result = await save_items(wcomponents_url, wcomponents, void 0);
  solid_dataset_cache.wcomponents_dataset = result.items_dataset;
  return result.error && Promise.reject(result.error);
}
async function save_items(items_URL, items, cached_items_dataset) {
